@guest
    @if (Route::has('register'))
        <a
            class="mr-2 mb-1 hidden rounded-sm bg-blue-100 px-4 py-2 text-xs font-bold text-blue-800 uppercase shadow-sm outline-hidden hover:shadow-md focus:outline-hidden active:bg-blue-200 md:block"
            href="/register"
        >{{ __('Register') }}</a>
    @endif
    <a
        class="mr-2 mb-1 rounded-sm bg-gray-700 px-4 py-2 text-xs font-bold text-gray-100 uppercase shadow-sm outline-hidden hover:shadow-md focus:outline-hidden"
        href="/login"
    >{{ __('Login') }}</a>
@else
