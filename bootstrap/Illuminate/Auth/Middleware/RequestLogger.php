...
final class RequestLogger
{
    ...
    private function generateResponseLogMessage($response, Request $request, float $duration): string
    {
        ...
        if ($status >= 300 && $status < 400) {
            ...
            if ($request->session()->has('errors')) {
                ...
                if (! empty($allErrors)) {
                    $validationErrors = ' - Validation Errors: ' . implode('; ', $allErrors);
                }
            }
        }
        ...
        $bodyAsJson = $request->expectsJson()
            ? json_encode(Arr::except(json_decode((string) $response->getContent(), true), []))
    ...
    {
        return array_any($this->excludePaths, fn (string $route): bool => $this->isRequestInRoute($request, $route));
    }
